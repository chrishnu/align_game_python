...
class MorabarabaAction(Action):
    ...
    def __init__(self, action_type, **kwargs):
        ...
        """
        assert isinstance(
            action_type, MorabarabaActionType), "Not a good action type format"
        self.action_type = action_type

        if action_type == MorabarabaActionType.ADD:
            assert ((len(kwargs) == 1) and ('to' in kwargs.keys())
                    ), "Need you from add only argument 'to'"
    ...
    :
            assert ((len(kwargs) == 1) and ('at' in kwargs.keys())
                    ), "Need you from add only argument 'at'"
